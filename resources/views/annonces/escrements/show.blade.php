        ...
        <div class="bg-gradient-to-r from-primary to-primary-dark py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center gap-4">
                    <a href="{{ route('annonces.escrements.index') }}" class="text-white hover:text-secondary-light transition">
                    ...
                    <div>
                        ...
                        <p class="text-white/80 text-sm mt-1">
                            ...
                            {{ $annonce->auteur->prenom ?? '' }} {{ $annonce->auteur->nom ?? 'Producteur' }}
            ...
            <div class="grid lg:grid-cols-3 gap-8">
                        ...
                        <div class="relative h-96 overflow-hidden">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center" style="background: linear-gradient(135deg, var(--color-secondary), var(--color-secondary-light));">
                                    <i class="fas fa-leaf text-8xl" style="color: var(--color-primary);"></i>
                                </div>
                            @endif
                ...
                <div>
                    <div class="rounded-xl shadow-lg p-6" style="background-color: var(--color-bg-white);">
                        ...
                        <div class="space-y-3">
                            @if($annonce->escrement)
                                <div class="flex items-center justify-between py-2 border-b" style="border-color: var(--color-nav-border);">
                                    <span class="text-sm" style="color: var(--color-nav-text); opacity: 0.7;">Type de fumier</span>
                                    <span class="font-semibold" style="color: var(--color-nav-text);">
                                        {{ $annonce->escrement->nom ?? 'Non spécifié' }}
                                    </span>
                                </div>
                                @if($annonce->escrement->espece)
                                    <div class="flex items-center justify-between py-2 border-b" style="border-color: var(--color-nav-border);">
                                        <span class="text-sm" style="color: var(--color-nav-text); opacity: 0.7;">Espèce source</span>
                                        <span class="font-semibold" style="color: var(--color-nav-text);">
                                            {{ $annonce->escrement->espece->label ?? 'Non spécifiée' }}
                                        </span>
                                    </div>
                                @endif
                            @endif
                            ...
                            <div class="flex items-center justify-between py-2 border-b" style="border-color: var(--color-nav-border);">
                                ...
                                <span class="font-semibold" style="color: var(--color-nav-text);">
                                    {{ $annonce->quantite ?? 0 }} kg
                                </span>
                            </div>
                            
                            <div class="flex items-center justify-between py-2 border-b" style="border-color: var(--color-nav-border);">
                                <span class="text-sm" style="color: var(--color-nav-text); opacity: 0.7;">Producteur</span>
                                <div class="flex items-center gap-2">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center" style="background-color: var(--color-secondary-light);">
                                        <i class="fas fa-tractor text-xs" style="color: var(--color-primary);"></i>
                                    </div>
                                    <span class="font-semibold" style="color: var(--color-nav-text);">
                                        {{ $annonce->auteur->prenom ?? '' }} {{ $annonce->auteur->nom ?? 'Producteur' }}
                                    </span>
            ...
            @if(isset($annoncesSimilaires) && $annoncesSimilaires->count() > 0)
                <div class="mt-12">
                    <h3 class="text-2xl font-bold mb-6" style="color: var(--color-nav-text);">
                        <i class="fas fa-leaf mr-2" style="color: var(--color-primary);"></i>
                        Annonces similaires
                    </h3>
                            ...
                            <div class="rounded-lg overflow-hidden shadow-md transition hover:shadow-xl" style="background-color: var(--color-bg-white);">
                                <div class="relative h-40 overflow-hidden">
                                    ...
                                    @else
                                        <div class="w-full h-full flex items-center justify-center" style="background-color: var(--color-secondary-light);">
                                            <i class="fas fa-leaf text-3xl" style="color: var(--color-primary);"></i>
                                        </div>
                                    @endif
                                ...
                                <div class="p-4">
                                    <h4 class="font-bold mb-1 line-clamp-1">
                                        <a href="{{ route('annonces.escrements.show', $similaire->id) }}" 
